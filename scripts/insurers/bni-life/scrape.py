...
RAW_DIR = PROJECT_ROOT / "data" / "raw" / "insurers" / "bni-life"

# BNI Life landing for individu-health is JS-only — listing API not in static HTML.
# Hardcode known products (only `spectra-health-care` is sold as individu kesehatan;
# `optima-group-health` is B2B group insurance, excluded).
LANDING_CFG = {
    "insurer_slug": "bni-life",
    "insurer_name": "PT BNI Life Insurance",
    "insurer_category": "asuransi_jiwa",
    "jenis": "konvensional",
}

PRODUCT_URLS = [
    "https://www.bni-life.co.id/id/perlindungan/detail/spectra-health-care",
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scrape BNI Life health insurance product pages and PDFs.")
    ...
    print(f"\n=== Scraping {len(PRODUCT_URLS)} BNI Life products (hardcoded list — JS-only landing) ===")
